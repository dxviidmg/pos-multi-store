...
import { getSalesDashboard } from "@/src/features/sales/api/sales";
import useTaskPolling from "@/src/shared/hooks/useTaskPolling";
import CountdownTimer from "@/src/shared/ui/CountdownTimer";
import LineChart from "@/src/features/admin/components/Dashboard/LineChart";
import DoughnutChart from "@/src/features/admin/components/Dashboard/DoughnutChart";
import SalesHeatmap from "@/src/features/admin/components/Dashboard/SalesHeatmap";
import AvgTicketChart from "@/src/features/admin/components/Dashboard/AvgTicketChart";
import KPICard from "@/src/features/admin/components/Dashboard/KPICard";
...
const Dashboard = () => {
  ...
  const kpis = useMemo(() => {
    ...
    salesData.forEach(s => { const store = s.store_name || "Sin tienda"; salesByStore[store] = (salesByStore[store] || 0) + val(s); });
    ...
    if (month === 0) {
      const byMonth = {};
      salesData.forEach(s => { const m = new Date(s.created_at).getMonth(); byMonth[m] = (byMonth[m] || 0) + val(s); });
      const entries = Object.entries(byMonth);
      bestPeriod = getTied(entries, k => MONTH_NAMES[k] || "N/A", "best");
      worstPeriod = getTied(entries, k => MONTH_NAMES[k] || "N/A", "worst");
    } else {
      const byDate = {};
      salesData.forEach(s => { const d = new Date(s.created_at).getDate(); byDate[d] = (byDate[d] || 0) + val(s); });
    ...
    // Por día de semana
    const byWeekday = {};
    salesData.forEach(s => { const d = new Date(s.created_at).getDay(); byWeekday[d] = (byWeekday[d] || 0) + val(s); });
    ...
    // Por hora
    const byHour = {};
    salesData.forEach(s => { const h = new Date(s.created_at).getHours(); byHour[h] = (byHour[h] || 0) + val(s); });
